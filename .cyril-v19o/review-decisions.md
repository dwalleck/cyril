...
`voice-discard-not-announced`, `voice-notice-emitted-on-both-paths`,
`paste-discard-not-logged`, `voice-discard-not-logged`,
`paste-log-fires-unconditionally`. The oracle is now invoked from `mutations.sh`
at all — previously the gate's `PASS C8` was never falsified by the mutation
set.
