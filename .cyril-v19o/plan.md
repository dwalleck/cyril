...
**Claim IDs:** [C5]
**Expected behavior:** `/powers` with no catalog held adds exactly one system line (prefix `No powers reported yet`, then the precondition: `--features kas` build + `--agent-engine kas` session) and opens nothing; with a catalog held it returns `ShowPowers { powers }` and the App opens the panel with those rows; `/powers <unrecognized argument>` adds a usage line and opens nothing; a `PowersChanged` push with no panel open leaves the panel closed while still updating the session catalog; with the panel open it replaces the contents; Esc closes it, arrows scroll one line, page keys scroll five (`MAX_VISIBLE_POWERS`), with the state clamping the jump to the window the placed popup actually got.
