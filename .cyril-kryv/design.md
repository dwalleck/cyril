...
| C13 | Structural locality | Engine-neutral summaries whose only difference is provider identity, plus crate ownership boundaries | Render identical Overview state with provider names `kiro` and `omp`; any metric-availability buffer difference falsifies provider neutrality, while Rust module visibility keeps wire JSON in converters/private sidecar code | Byte-identical Ratatui `TestBackend` buffers after changing only provider identity; unlike the removed substring scan, the oracle exercises observable behavior and survives file moves | `widgets/usage_panel.rs`: branch on provider `kiro` for placeholders; provider-invariance buffers differ | `usage_panel::tests::overview_metric_availability_is_provider_invariant` plus crate visibility | <1s | PASS |
...
- 2026-08-22 — Review finding F16 demonstrated that the cross-crate `include_str!` substring scan was package-fragile and trivially evaded. Replaced it with provider-invariance rendering over byte-identical buffers; `cargo test -p cyril-ui overview_metric_availability_is_provider_invariant` passed.
...
Review amendment: F16 replaces the approved C13 source-substring oracle with the provider-invariance behavioral oracle above; the structural claim and accepted risk set are unchanged.
