...
- **C9** — At 100,000 rows across ≥ 20 `(provider, model)` groups, the new statistics add ≤ 700 ms to `snapshot()`. *(Bound revised from 250 ms on 2026-08-28 after measurement; see `spec.md`.)*
...
| C9 | ≤ 700 ms added at 100k rows / ≥20 groups | S16 | Generate 100,000 rows across 20 `(provider, model)` groups; time `snapshot()` with the new statistics and with them compiled out; assert the delta ≤ 700 ms. Falsified above the bound. Other cause: a warm page cache flattering the run — excluded by timing both variants in the same process after an identical warm-up query | Wall-clock measurement of the two variants; independent of the correctness oracle | In `usage.rs`, replace the single grouped percentile subquery with a correlated per-row subquery. Expected red: the measured delta exceeds 250 ms | `usage::tests::grouped_percentile_stays_within_budget` (deterministic assertion of the measured bound) | ~10 min incl. fixture generation | PENDING — checkpointed-build, per-slice gate (slice assigned in plan.md) |
...
- **cyril-nanu** — `snapshot()` recomputes every rollup on each context sample. Filed during the build: the baseline already costs ~342 ms at 100k rows on the always-on `refresh_usage_panel_from_log()` path, independent of this change. This is the defect the 250 ms budget was really pointing at.
...
Re-approval after C9 revision (verbatim): "Raise budget + ticket the refresh"
Date: 2026-08-28
Approved risk acceptances: None — C9 keeps a deterministic fence, now asserting the revised 700 ms bound. The accepted cost is recorded in `spec.md`'s revised criterion and tracked at cyril-nanu. — every claim carries a deterministic regression fence; no row uses `N/A — approved risk`.
