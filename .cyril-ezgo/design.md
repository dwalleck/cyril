...
- **Owner:** the bridge owns exactly-once attempt state and wire prefixing; `ProjectMemory` owns context selection/rendering; `App` continues to own visible original-user-message projection.
- **Competing seam A (selected):** a narrow core-owned `FirstPromptContext` interface has one async method accepting original first user text and a character budget and returning a complete optional block. A binary adapter wraps the already-bound concrete `ProjectMemory`; a fake adapter drives bridge tests. This is an adapter seam, not a selectable memory backend.
...
- **Competing seam C (rejected):** prepend in `App` before `SendPrompt`. That misses deferred/core-created prompt paths, couples visible and wire content, and puts the interim adapter at the wrong altitude under ADR-0003.
...
For each new/reused session ID accepted from `NewSession`, bridge state sets `memory_attempted = false`. On that session's first accepted `SendPrompt`, it flips the flag **before** awaiting memory, passes only the first original text block as query context, and applies a fixed 250 ms deadline with a 4,000-character budget. A returned block becomes a new first ACP text block; every original block follows unchanged. `None`, disabled state, channel closure, error, or deadline expiry sends the original vector unchanged. Empty original vectors are unchanged and still consume the one attempt. Later prompts never call the adapter.
