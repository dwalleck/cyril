...
(Corrected 2026-08-09: dispatch is engine-owned, not kiro-owned — `protocol/engine.rs` `convert_ext_notification` tries `convert::kas::workflow::to_notification` first and falls through to `convert::kiro::to_ext_notification` on `NotWorkflow`; `to_ext_notification` itself does not delegate to this adapter.)
...
(Superseded 2026-08-09 (CR1): the shipped vendor flattener H1n is the contract — a present `iteration` wins outright, else trailing ASCII `#digits` rewrites verbatim; child type and parent id are not consulted. See review-decisions.md.)
...
5. Repeat wrappers translate only for a direct sequence child matching both `<repeat-id>#N` and `iteration == N`; they yield `iter-N` while preserving wrapper metadata, and every near miss remains literal. (Superseded 2026-08-09 (CR1): the shipped vendor flattener H1n is the contract — a present `iteration` wins outright, else trailing ASCII `#digits` rewrites verbatim; child type and parent id are not consulted. See review-decisions.md.)
