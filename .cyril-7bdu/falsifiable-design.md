...
| 4 | slow resolver doesn't starve loop | Resolver where req A `sleep(500ms)`, req B instant; send A then B; if B's response is delayed until A completes (serialized) → false | timing/order of the two oneshot resolutions, observed by the test | 1.5h | pending | integration `host_io_slow_request_does_not_block_loop` |
