...
| 6 | Replays stay idempotent and terminal behavior unchanged. | Apply each sequence twice, then apply completed/failed/aborted snapshots. Falsified by a second-pass change or a non-absorbing terminal state. | Existing cyril-6beh replay oracle and terminal status matrix. | 2 min | pending | New ordering branch special-cases first delivery or changes `is_terminal`. | Existing replay/terminal tests plus the ordering matrix's second pass. |
