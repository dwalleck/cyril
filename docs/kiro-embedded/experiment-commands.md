enabledâ  Experimental features may be changed or removed at any time

 experiment clearagentcontexteditorreplycompacttoolsissuelogdumpchangelogpromptshooksusagemcpmodelexperimenttangentcheckpointtodospastekiro_plannerAlready in Plan agent. Use Shift+Tab to return to previous agent.

â  The /save command is deprecated. Use /chat save instead.

â  The /load command is deprecated. Use /chat load instead.
addto addEscâto saveYour administrator has configured an MCP registry, and you can only install servers from that registry. Use 'mcp add' to install registry servers.No active agentAll registry servers are already enabled.

 added
removeto remove removed

--- tools refreshed ---
No MCP servers installed

â  OAuth token expired or invalid: To learn how to configure MCP servers, visit 
  Token will be refreshed on next tool call.
https://kiro.dev/docs/cli/mcpServer is still initializing...

â[loading]: Failed to save agent config: Failed to reload servers: Changes were not saved.

â  Servers reloaded but no tools were loaded

â ï¸  Warning: Could not save to disk:  for server status and errors

â Servers reloaded successfully ( MCP tools loaded)
Changes to  agent only apply to current session.

â ï¸  Note: Agent has no config file.

ð Reloading MCP servers...
Changes are in memory only and will be lost on restart.
Failed to serialize agent: Changes were saved to disk. Restart the session to apply them.

   Check 
â ï¸  Error reloading servers: â (v] âð MCP Registry Servers
To configure MCP servers, use ð Configured MCP Servers
    Command:  or visit No MCP servers configured.

Generating codebase overview for summary...# Codebase Summary
