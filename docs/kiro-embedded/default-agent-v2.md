...
%Y-%m-%d %H:%M:%S- ID: 
  ...
  Last Updated: 
  Items: 
Status unavailable: Contexts unavailable: Failed to get contexts: 
Failed to get status: Failed to access knowledge base: Path '' does not exist, will try to remove by path string matchPlease provide at least one of: name, context_id, or pathPlease confirm clearing knowledge base by setting confirm=truePlease provide either context_id, name, or path to identify the knowledge base entry to updateFailed to build HTTP clientHTTP error text/Unsupported content type: . Expected text/html or text/*Response too large:  bytes (max:  bytes)Failed to read response bodyselectivetruncated contentfull content) from URLURL cannot be emptyInvalid URL format: Received non-object JSON for tool arguments: WARNING: the actual tool use arguments were not a valid JSON objectExpected JSON object, got: WARNING: the actual tool use arguments were too complicated to be generatedReceived an unexpected end of stream after spending ~s receiving tool eventsAttempting to recv next eventReceived new eventGenerateAssistanceResponse - MeteringEventreceived unexpected event from the response streaminvalid state eventreceived unexpected event type in main parsing loopGenerated new message idsending send_message requestsend_message succeededCould not determine home directorySkipping file 'No files found matching glob pattern 'No readable files found matching glob pattern '' ( skipped)Glob entry error: Skipping context path 'Rule '' already exists.Invalid path '. Use --force to add anyway.mcpServersNo mcpServers field found in configPath must be a directory already exists. AbortingNo agent with name '' foundMigration did not happen for the following reason: Agent config  is malformed at Global MCP config not found: Error loading global mcp config from Error loading workspace mcp config from Failed to parse MCP config workspace directory.
 was found in the Duplicate agent with name skill://.kiro/skills/*/SKILL.mdskill://~/.kiro/skills/*/SKILL.mdvalid resourcefile://{}/**/*.mdfile://AmazonQ.mdAgent name field does not match filename, skipping direct matchFailed to parse agent JSON from fileFailed to read agent fileFailed to parse agent JSON while searching by nameFailed to read agent file while searching by name does not existFailed to read agents directoryMigration was not performedProfile migration successfulError creating global agent dir: agent_config.json.exampleError obtaining example agent path.Error creating example agent config: This is an example agent config (and will not be loaded unless you change it to have .json extension)mcp_tool_name@mcp_server_name_without_tool_specification_to_include_all_toolsError serializing example agent configError writing example agent config to file: # Kiro CLI Default Agent
