...
Error converting active agent  to value for validation. SkippingSkipping config validation because there is no active agentFailed to convert agent definition to schema: . Skipping validation: user defined default  not found. Falling back to in-memory defaultFailed to sync resources for active agent: â ï¸  WARNING: : no agent with name  found. Falling back to user specified defaultNo agent with name  foundcontext.jsonMalformed legacy global mcp config detected: . Skipping mcp migration.Legacy profiles detected. Would you like to migrate them?Failed to choose an option: migrated_agent_from_global_contextThis is an agent migrated from global contextAborting migrationNothing to migrateThis is an agent migrated from profile context persisted in path  does not have path associated and is thus not migrated.Issue TitleCurrent version: Detected platform: Using manifest URL: Checking for updates...
Update available: --force specified, proceeding with installation anyway
You are on a newer version () than the latest release (Downloading installer...
Installing update...
Downloading: %No artifact found for tmpkiro-installer-Failed to delete file after checksum mismatch: macosRunning installer kind= path=Unsupported installer kind: Linux DEB auto-install is not yet implemented. Linux updates are managed by the desktop app.MSI installation not supported on this platform. Would run: msiexec /i  /quiet /norestartMSI installation is only supported on WindowsPKG installation is only supported on macOSTar-based auto-install is not yet implemented. Updates on this platform are managed by the desktop app.No value associated with null
