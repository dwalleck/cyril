...
**Cyril impact:**
- *Passive:* none. The three `steering_*` variants ride `kiro.dev/session/update`; cyril's converter (`convert/kiro.rs` `kiro.dev/session/update` arm) returns a Protocol `Err` for unknown variants — but the variants are only ever emitted as echoes of a client's own steer request, which today's cyril never sends.
