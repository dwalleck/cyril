...
> The read-only subset (`permissions/list`, `permissions/explain`, `policy/check`, `codeIntelligence`, `session/context`, `session/history`) is **verified live** — see the 2.7.1 audit's "Client→agent methods" capture (incl. the default Cedar policy ruleset and the LSP-server matrix). `policy/check` resolves an `ask` effect by firing `session/request_permission`. The **full spec workflow** is verified — `spec/resolveSession` → `spec/invoke {operation:'createSpec'}` (requirements) → `spec/invoke {operation:'generateDocument', documentType:'design'|'tasks', specDocuments:[…], action:'create'}` (design, then tasks) → `spec/getTaskStatuses` (hierarchical `{taskId, markdownStatus, isLeaf, isOptional, subTasks[]}` tree). Each `invoke` is **async** (returns `{sessionId}`, then streams a full turn), self-scaffolding `.kiro/specs/<feature>/{requirements,design,tasks}.md` via bundled subagents. **`executeTask` is also verified** — `{operation:'executeTask', tasksFilePath, taskId}` (taskId = the heading text from `getTaskStatuses`) returns `{sessionId, executionId}` (the only spec op carrying an `executionId`), delegates to the bundled `spec-task-execution` subagent, **writes real source**, and flips the task to `markdownStatus:"completed", executionStatus:"succeed"`. See the audit "spec workflow" capture + the sample doc set + implemented source in `experiments/conductor-spike/spec-sample-2.7.1/`. **`runAllTasks` is now also verified** (2026-07-05, live on 2.11.0/@kiro/agent 0.8.0, `experiments/conductor-spike/probe-kas-runalltasks-2.11.0.py` + `kas-runalltasks-dumps/`): accepts `{operation:'runAllTasks', sessionId, featureName, tasksFilePath, specDocuments}` → `{sessionId, executionId}` then ONE long spec-mode turn (“Run all tasks for this spec”, per the bundle: checkpoint/resume via `TaskMetadataStorage`, `makeAllRequired` strips optional markers). Execution is SEQUENTIAL per-task: one `Sub-agent: spec-task-execution` (`_meta.kiro.kind: agent-subtask`) per task in ready order — not a parallel fan-out. Both probe tasks completed (`markdownStatus:'completed', executionStatus:'succeed'`, checkboxes flipped, `tasks.meta.json` sidecar appears, `lastSessionId`/`lastExecutionId` stamped); the agent self-verified by RUNNING the code and the new test. Permissions still fire for every write/shell step (4 in the probe) — consistent with the 2.10.0 spec-flow finding. Works against a HAND-SEEDED spec (no createSpec/generateDocument needed — `getTaskStatuses` parse-validates a hand-written tasks.md).
