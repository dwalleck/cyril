...
- **Net for a token-usage feature**: three non-modification observation paths exist — (a) this OTLP redirect, (b) TLS interception of the CodeWhisperer/KRS stream to read raw `metadataEvent.tokenUsage`, (c) a fresh AWS prompt-log pull — but all three are limited by the same backend gate. The blocker is upstream emission, not KAS concealment. Re-probe when a model/backend known to emit tokenUsage is available (some models may populate it; `model=auto` today did not).
