...
> **Spec workflow (verified live 2026-06-16, but out of scope as a built UI):** `_kiro/spec/resolveSession` + `_kiro/spec/invoke {operation:'createSpec'}` work — async (return `{sessionId}`, then stream a full turn), self-scaffolding `.kiro/specs/<feature>/{requirements.md,.config.kiro}` (EARS-format requirements; `createSpec` = requirements phase only, design/tasks via `generateDocument`) using bundled subagents (`feature-requirements-first-workflow`, `requirement-detailer`) + interactive questions. If cyril ever surfaces spec, it rides the **KAS-3 agent-subtask rendering** + the `pending_interaction`/`userInput` path — no new primitive. See the 2.7.1 audit "spec workflow" capture and `experiments/conductor-spike/spec-sample-2.7.1/`.
