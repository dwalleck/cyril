...
Build `transcript-recorder` as Cyril's first stage. The conductor foundation is already production-active: every agent connection uses `agent-client-protocol` 2.0 and `agent-client-protocol-conductor` 2.0, including the zero-stage case. The first stage therefore extends `StageChain`; it does not add a feature-gated runtime or restore a direct-spawn bypass.
...
**Status note (updated 2026-08-30):** the conductor-first SDK2 runtime is complete (`cyril-gl5s`); the first actual proxy stage remains on hold behind the KAS host-callback track (Open Tension #8, ADR-0003). KAS fs/terminal/hooks are still the near-term side-effect interception seam. The first wire stage should target a need host callbacks cannot satisfy—fan-out/observer or cross-vendor workflow orchestration—and use the official SDK2 component roles already owned by `SdkRuntime`.
...
- **Inspection workbench (`cyril-ukmu`)** — Phase 5+ platform capability using
  the existing KAS authentication/runtime and native policy boundaries, not a
  new proxy stage. Child `cyril-jlxx` ([PR #116](https://github.com/dwalleck/cyril/pull/116))
  supplies the `cyril-workbench` inspection-only reviewer backend: staged
  evidence, isolated native configuration, confirmed model selection and
  owned run lifecycle. Its [archived evidence](../.cyril-jlxx/evidence.md)
  establishes original CLI 2.21.1 Linux behavior only. This does not complete
  the parent workbench: desktop/Evidence desk (`cyril-9qn7`), queue authorization
  (`cyril-u7qp`), Findings/coverage (`cyril-3qva`), provider/publication acceptance
  and native Windows configured enforcement (`cyril-6y1s`) remain separate gates.
...
> **Update 2026-07-15 — 2.12.3 / KAS 0.17.2 audit: the cloud-agent stack lands (dormant).** Full audit `docs/kiro-2.12.3-wire-audit.md`. Unlike the 0.3.299→0.8.0 renumber, **`@kiro/agent` 0.8.0 → 0.17.2 is a real jump** — five weeks of work shipped after three byte-frozen releases. It is the **client-side plumbing for Kiro cloud/remote agents**, complete and visible in the handshake but inert on a standalone CLI. **v2 is frozen** (same-day A/B zero drift) so this is KAS-only, and none of it blocks the walking skeleton — but it reshapes the back half of the track. Milestone-mapped:
> - **KAS-1 auth contract UNCHANGED — the one that could have bitten didn't.** The `RequestCredentialResolver` refactor across all four auth providers is an internal atomicity fix (one ensure-then-read snapshot so a refreshed token can't pair with a stale profileArn — exactly the failure mode in `reference_kiro_kas_launch_contract`). The `_kiro/auth/getAccessToken` reply fields cyril must supply are **byte-identical** to 2.12.1 (`accessToken`/`expiresAt`/`profileArn`, `provider`/`authMethod` read both). **Free path re-confirmed on 0.17.2** (`probe-webportal-capture-2.12.3.py`: standalone spawn, no `--auth`, `getAccessToken` callbacks = 0). KAS-1 as scoped stands.
> - **A cloud-agent capability layer appears — gated behind one env var.** `KIRO_REMOTE_SESSIONS_ENDPOINT` (or `--remote-sessions-endpoint`) flips **four new `_meta.kiro` initialize caps at once** — `sourceProviders`, `sessionSources` (+`remote`), `sessionListScopes` (+`user`), `executionTargets` (+`cloud-sandbox`) — and advertises two new methods `_kiro/sourceProviders/{list,listResources}` (repo-provider catalog: **GITHUB / GITLAB / MIDWAY**, MIDWAY = internal-Amazon). Sessions gain a persisted `executionTarget` (`local` = `executedHere` vs `cloud-sandbox` = **`relayed`**). The dispatch is unconditional (unadvertised call → typed catalog error, not `-32601`); the real gate is the endpoint. Modeling → **cyril-tikf** (the 4 caps). This is what a future "run this agent in the cloud" feature is built on; cyril treats it as **KAS-8 / deferred** until there is a reason to speak it.
> - **The web-portal wire contract is captured** (first time): `@amzn/kiro-web-portal-service` = Smithy **RPC-v2-CBOR** over `POST /service/KiroWebPortalService/operation/<Op>` (ACP `sourceProviders/list`→`ListAvailableProviders`, `listResources`→`ListProviderResources`), auth via three headers `Authorization: Bearer` + `X-Kiro-Idp` + `X-Kiro-Profile-Arn`. Distinct backend from the CodeWhisperer streaming turn endpoint. If cyril ever drives cloud sessions directly, this is the credential contract.
> - **`_kiro/session/list` response shape CHANGED → cyril-nn85 (future modeling).** cyril doesn't call `session/list` today. 2.12.1 returned bare summaries; 2.12.3 returns `{sessions:[{sessionId,cwd,title,updatedAt,_meta.kiro:{source,executionTarget,status,agentMode,createdAt,parentSessionId?,description?}}], _meta?.kiro.warnings}`. Request adds optional **gated** `_meta.kiro.{sessionSource,listScope,executionTarget}` (non-default → InvalidParams unless `remoteConfigured`) — additive/safe. Fold into KAS session-management when it lands.
> - **`_kiro/system/notify {level,message}` promoted from "tolerated" → cyril-08eh.** Trigger confirmed = `QClientRetryStrategy` model-request backoff (levels `info`/`warning`, canned messages), **not** remote-only — a KAS turn hits it in ordinary slow/throttled use. Surface as a system line; belongs in KAS-8.
> - **Not wire changes (no issue, recorded for completeness):** `sessionSubscriberCount` (internal `SessionOutbound` port — relayed session checks if a client is attached), and `tree-sitter-powershell` + `node-addon-api`/`node-gyp-build` (off-wire native PowerShell AST parser for command-safety classification — changes *when* `session/request_permission` fires, not its shape; a **Windows-shell-support signal** relevant to KAS-5's `platform/path.rs` cross-platform work, not the parser).

**Why its own track (not a K-item):** the K-track is feature-parity within the v2 engine cyril already drives; KAS is a parallel engine with its own dialect and lifecycle. It also intersects the platform vision — KAS is the first Kiro engine that exposes filesystem callbacks, which is a genuine proxy-stage interception point (links to Phase 5). A second intersection: KAS's `multiplex-stream.d.ts` is a worked reference design for the platform-vision **Fan-out observer** stage — the `toolCallId`-keyed multi-client observer pattern cyril would steal is tracked as **cyril-5g2o** (see the Fan-out observer bullet above and `docs/kiro-2.14.1-wire-audit.md`); it is a platform-stage item, not a KAS-N engine milestone.
...
**Architecture (see [ADR-0001](adr/0001-kiro-engine-trait.md), [ADR-0002](adr/0002-kas-cargo-feature-gate.md)):** engine is bound at agent-process spawn; every connection crosses `AgentProcess → ConductorImpl → SDK Client handlers → DomainMediator`. The serial mediator owns one `Rc<dyn Engine>` plus its adapter selection, so every session on the process shares the same engine and switching requires a process restart. Client handlers only enqueue owned work; Engine conversion and KAS auth/fs/terminal/hooks effects remain mediator-owned.
...
**ACP SDK coverage (updated 2026-08-30):** Cyril pins `agent-client-protocol` 2.0.0, transitive schema 1.5.0, and `agent-client-protocol-conductor` 2.0.0 while negotiating stable wire v1. KAS `session_info_update` remains represented by the stable-v1 schema. Because `SessionUpdate` is strictly tagged without a serde catch-all, the SDK Client installs an untyped `session/update` fence before the typed handler; future variants no longer close the connection.
...
| `system/notify {level,message}` (2.12.3; fires on `QClientRetryStrategy` model backoff, `info`/`warning` — normal use, not remote-only) | ❌ cyril-08eh — surface as system line |
| `powers/items_changed` · `progressive_context/items_changed` · `sessions/changed` · `tools/didChange` | ⚠ tolerated; surface opportunistically |
...
**Currency caveat:** the covenant *doc* catalog is synced at `@kiro/agent` 0.3.224; the installed package is now **0.17.2 (2.12.3)** — a large drift. As of the 2.12.3 audit the uncataloged surface added by the cloud-agent stack is: `sourceProviders/{list,listResources}`, the reshaped `session/list`, `system/notify`, and four new `_meta.kiro` handshake caps (`sourceProviders`/`sessionSources`/`sessionListScopes`/`executionTargets`) — all mapped above (cyril-tikf/nn85/08eh). **Re-sync the covenant** (README step 6) before relying on this matrix; the jump from 0.3.257 → 0.17.2 is overdue for a full catalog refresh.
...
- **Historical ACP-coverage verification spike:** schema 0.11.2 covered the KAS variants emitted during KAS-0. Production now uses the SDK2/schema 1.5 contract and unknown-update fence described in the current architecture above.
...
> **Implemented 2026-07-19 (cyril-jiyn, PR pending).** The `enabled` (host) vs `v2` (agent-side) models **do not compose** — live A/B on 2.13.0 (`.cyril-jiyn/probe-hooks-ab-2.13.0.py`) proved KAS's `buildSessionHooks` is winner-take-all per session: with `{enabled:true,v2:true}` the standalone loader ran the on-disk hook agent-side and drove **zero** host callbacks. So the choice is a config knob **`[agent] kas_hooks = "host" | "kas" | "off"`, default `host`** (cyril as the org-policy executor/gate; the reason KAS-7 exists). Shipped: the advertisement per mode, a `HookRegistry` (loads workspace+global `.kiro/hooks`, PascalCase→camelCase trigger map, skip-warn on invalid), and the `_kiro/hooks/{list,executeHook,sessionStart,cancel,didChange}` responders including the exit-2 preToolUse block and operationId cancel/timeout-kill. Shipped additionally 2026-07-23: sessionStart precomputed-context execution (cyril-tpfd — the `AcpPrecomputedHookResult` shape was carved from the bundle's own producer/consumer and live-verified; `.cyril-tpfd/findings.md`). Deferred: registry hot-reload (cyril-2adk), agent-type hook actions (cyril-n03f), hooks-panel UI (cyril-oiyt). **Per-release fence:** re-run the A/B probe (see `experiments/conductor-spike/README.md`).
...
- Advertise `clientCapabilities._meta.kiro.hooks = {enabled: true}` at `initialize` (sibling of `settings`). **2.13.0 update (wire-verified 2026-07-17, `probe-kas-hook-confirm-2.13.0.py`):** the "no `v2`" note that stood here is stale — the hooks object now accepts an optional `v2: true`, which activates a *second, agent-side* hook model: KAS's standalone `.kiro/hooks/*.json` loader (kasHookFileSchema, file-watched, workspace + global roots), Stop-hook `confirm` dialogs (`_meta.kiro.hookConfirm` → `session/request_permission`, at most one per turn), and the `CreateHookTool`. With `{enabled: true, v2: true}` the live capture drove **zero** `_kiro/hooks/*` host callbacks — `v2` shifts hook ownership from cyril to KAS, so enabling it trades the org-policy gate below for on-disk user hooks + confirm dialogs. Decide the combination in cyril-jiyn before implementing.
...
4. **No standardized stage registry exists yet.** The runtime now uses the official Rust SDK2 `ConnectTo`/`Conductor` component ABI, so a future registry can select stage components without another transport rewrite.
5. **Upstream SDK2 is young.** Cyril pins exact SDK/core/conductor versions and keeps stable wire v1 as the compatibility boundary; upgrades need the deterministic conductor and authenticated v2/KAS acceptance matrix.
...
8. **Proxy stages remain deferred; the conductor runtime does not** (updated 2026-08-30; see [ADR-0003](adr/0003-defer-proxy-stack-for-host-callbacks.md)). Every connection already crosses the official zero-or-more-stage conductor. KAS host callbacks remain the near-term side-effect interception path; a non-empty `StageChain` waits for fan-out/observer or cross-vendor orchestration that host callbacks cannot provide.
...
- [agentclientprotocol/rust-sdk](https://github.com/agentclientprotocol/rust-sdk) — production SDK2 component and conductor implementation
- [agentclientprotocol/symposium-acp](https://github.com/agentclientprotocol/symposium-acp) — historical `sacp` prototype evaluated by the conductor spike
