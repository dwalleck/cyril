...
1. **`session/set_model` — RESOLVED 2026-08-30 against 2.20.1; cyril's doc was HALF wrong.**
   Probe `experiments/conductor-spike/probe-v2-set-model-2.20.1.py`, capture `v2-set-model-2.20.1.jsonl`.
   - **It works.** A served id is acked in **1.4 ms** with an empty `{}` result, and the session sidecar
     (`$HOME/.kiro/sessions/cli/<sid>.json` — the only on-disk record of the billed model) confirms the
     paid turn ran on `claude-opus-5`. KiroCrew's 2.15.1 finding holds at 2.20.1.
   - **But "not advertised in capabilities" was CORRECT** — `agentCapabilities.sessionCapabilities` is
     `{}`. The doc's error was inferring *unusable* from *unadvertised*. That is the reusable lesson:
     absence from capabilities is not absence of the method.
   - **It does NOT validate.** A bogus `modelId` is accepted in **0.4 ms** with `{}`. The *next*
     `session/prompt` fails in **0.4 s** with `-32603` and **zero** `session/update` frames:
     `"Encountered an error in the response stream: The model 'definitely-not-a-model-9x' is not
     available. Please use '/model' to select a different model and try again. (request_id: …)"`
     Upgrades `cyril-fj6j` from FIRST-PARTY to **MEASURED**.
   - **Refines §4.3's envelope finding:** this is the *named* rejection. KiroCrew reports ≥2.16 reworded
     the **capacity** rejection to a *nameless* form ("The model you've selected is temporarily
     unavailable"). Two failures — one terminal, one transient — ride the same
     `Encountered an error in the response stream:` envelope, so classifying on the envelope conflates them.
2. **`configOptions` — RESOLVED; cyril was RIGHT, and more precise than KiroCrew.** On 2.20.1 the v2
   `session/new` result carries only `['models', 'modes', 'sessionId']`: the key is **absent entirely**,
   not `null`. `session/set_config_option` returns `-32601 Method not found`, `data:
   "session/set_config_option"`. KiroCrew's both-backends `configOptions` parsing is a claude-adapter
   path, not a kiro one. No change needed to `cyril-cxwb` (which covers the KAS half).
...
### 9.1 Live 2.20.1 model catalog (captured incidentally)

v2 `session/new` **does** return `models {currentModelId, availableModels[{modelId, name, description}]}`
— 19 entries — and `description` carries the **context window in prose**:

```
auto                Models chosen by task for optimal usage and consistent quality
claude-opus-5       Claude Opus 5 model with 1M context window
claude-sonnet-5     Claude Sonnet 5 model with 1M context window
claude-opus-4.8     Claude Opus 4.8 model with 1M context window
gpt-5.6-sol         Experimental preview of OpenAI GPT 5.6 Sol with 272k context window
gpt-5.6-terra       Experimental preview of OpenAI GPT 5.6 Terra with 272k context window
gpt-5.6-luna        Experimental preview of OpenAI GPT 5.6 Luna with 272k context window
claude-opus-4.7 / -4.6 / claude-sonnet-4.6     ... with 1M context window
claude-opus-4.5 / claude-sonnet-4.5            (no window stated)
claude-sonnet-4     Hybrid reasoning and coding for regular use
claude-haiku-4.5    The latest Claude Haiku model
deepseek-3.2 / minimax-m2.5 / minimax-m2.1 / glm-5 / qwen3-coder-next
```

- **A model→window map is reachable without `--list-models`**, straight off `session/new`. Softens
  `cyril-qy6v` from "required" to "nice to have" for the *window* half; the **credit multiplier** still
  needs `--list-models` — it is absent from the ACP payload.
- **`gpt-5.6-sol` = 272k confirmed live**, matching the KiroCrew website fixture exactly — independent
  evidence their fixtures are captured, not invented.
- `gpt-5.6-terra` and `gpt-5.6-luna` are served models cyril had no record of.
- `commands/options` entries carry **no `current` flag** (`cyril-imjx` holds at 2.20.1). v2 `modes`
  advertises `kiro_default` + `kiro_planner`.
