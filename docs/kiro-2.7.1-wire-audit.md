...
- **cyril ships this host (2026-07, cyril-jiyn / ROADMAP KAS-7) — decided default `kas_hooks = "host"`.** The `[agent] kas_hooks` knob: `host` = advertise `{enabled: true}` (cyril owns the `.kiro/hooks` registry + executor — the org-gate model); `kas` = advertise `{enabled: true, v2: true}` (KAS's standalone agent-side loader + confirm dialogs); `off` = key absent. **The two models do NOT compose — KAS's `buildSessionHooks` is winner-take-all:** the 2.13.0 A/B (`experiments/conductor-spike/probe-hooks-ab-2.13.0.py`, results in `.cyril-jiyn/ab-results-{host,v2}`) shows `enabled+v2` drives ZERO `_kiro/hooks/*` host callbacks (hooks run agent-side, outside cyril's gate) while enabled-only drives the host-callback set — so `host` it is. Per-release fence: re-run the probe both arms (`experiments/conductor-spike/README.md`).
- **sessionStart precomputed context SHIPPED (2026-07-23, cyril-tpfd).** cyril executes the registry's SessionStart command hooks (sequentially, per-hook file `timeout` seconds, default 60, `USER_PROMPT` present-but-empty) and replies `{results: AcpPrecomputedHookResult[]}`. **Carved element shape** (identical 2.13.0/2.14.1 — producer and consumer both live in `acp-server.js`): `{id, name, hookId, originalType: "runCommand" | "askAgent", content}`; an unknown `originalType` **throws** (`assertNever`) in the agent's telemetry path. Consumption: KAS wraps each `content` in a `<HOOK_INSTRUCTION>` block appended to the session's first user prompt (runCommand content additionally gets a `[Session Start Hook Output]` banner); the producer's packaging is `stdout || stderr` (stdout-else-stderr, NOT combined) with empty-output hooks dropped — cyril matches both. Live-observed caveat: an unbriefed model (non-`kiro-ide` clientInfo → no hooks briefing) may treat injected `HOOK_INSTRUCTION` text as untrusted and decline to obey it. Full carve + live injection MATCH: `.cyril-tpfd/findings.md`. Covenant-doc re-sync of the shape stays cyril-mfkg.
...
### fs + terminal host callbacks (verified live — the KAS-5 interception point)

`probe-kas-fs-terminal-host-2.7.1.py` advertised `clientCapabilities.fs = {readTextFile:true, writeTextFile:true}` and `terminal:true`, implemented **real** responders (read/write files on disk, spawn processes), and ran one turn that read a file, wrote a file, and ran a shell command. **KAS routed all three through the client** — it did *not* touch disk or shell itself:

- **fs:** `fs/read_text_file` (README.md → cyril returned the content; the agent learned a value it could only get from our callback) → `fs/read_text_file` notes.txt (returns **not-found** — the agent checks before writing) → `fs/write_text_file` notes.txt (cyril wrote it) → `fs/read_text_file` notes.txt again (verify-after-write). **`notes.txt` exists on disk with the exact content *because cyril's responder wrote it*.**
...
- **Settings / modes not exercised:** KAS modes `autonomous` / `bug-fix` / `plan` / `quick-spec` (**update 2026-07-02:** `plan`/`bug-fix`/`quick-spec` now live-exercised on 2.10.0 — see [kiro-kas-modes-2.10.0.md](kiro-kas-modes-2.10.0.md); `autonomous` still open); most `AgentSettings` flags (`_parallelTasks`, `_steeringReminders`, `_sessionRecap`, `_mergeVibeSpec`, `_delegate`, `_providerPowers`, `thinking`, `tangentMode`, `disableAutoCompaction`, `todoList`, `checkpoint`, `toolSearch`, `knowledge`, `compaction`); the `c2s_*` code-to-spec tools (gated `_c2s`); the knowledge base.
