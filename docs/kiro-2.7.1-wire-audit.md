...
- *Empirical status:* enable path verified (`hooks/list` stops erroring once `_meta.kiro.hooks.enabled` is set). End-to-end firing **not** reproduced — my probe answered KAS's `_kiro/hooks/list` callback with an empty `{}` and omitted the required `trigger` param, so no hook was ever returned to execute. A faithful repro needs cyril to *act as the hooks host* (return a hook from `list`, then handle the `executeHook` callback).
