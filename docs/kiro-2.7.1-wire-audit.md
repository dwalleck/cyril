...
**Gating + direction (verified live 2026-06-16, `probe-kas-hooks-enabled-2.7.1.py`):** the whole hooks surface is **gated behind a client-supplied flag** — `_kiro/hooks/list` returns `-32603 "_kiro/hooks/list is not available when v2Hooks is disabled"` until enabled. The **enable path is `clientCapabilities._meta.kiro.hooks = {enabled: true, v2: true}`** at `initialize` — a **sibling of `_meta.kiro.settings`, not inside it** (bundle: `if (kiroMeta?.hooks?.enabled) { if (hooksConfig.v2 === true) { … new HooksModuleCache(…) } }`). So it **is** session/client-settable, like `subagentOrchestration` — the trap is the *location*: `_meta.kiro.settings.{v2Hooks|hooks}` does **not** work, and neither does a `hooks` block in `~/.kiro/settings/cli.json` (both verified negative). With the correct flag, `_kiro/hooks/list` returns `{hooks: [...]}`. **Direction:** the bundle runs hooks **server-side** — command-action hooks via `CommandAction({processRunner})`, agent-action hooks against KAS's own model; the only server→client hook push is `_kiro/hooks/cancel` (fired on `session/cancel` when hooks are enabled), and the host-facing API (`list`/`executeHook`/`triggerHook`) is **client→server requests**. There is no "host runs this hook for me" callback. *(Caveat: not fully reproduced end-to-end — a workspace `.kiro/hooks/*.json` placed in the probe's temp repo did **not** appear in `hooks/list` or fire on a tool-use turn, so hook **discovery/loading** has an unresolved nuance — workspace recognition, a global `~/.kiro/hooks/` location, or trigger/schema specifics. The server-run direction is read from the bundle, not yet observed firing.)* For cyril, a hooks stage means *enabling + driving + observing* the engine's hooks, not implementing a host-run callback responder.
...
- **Hooks become a real ACP surface (reverses prior guidance).** The standing "don't implement hooks; they're backend-only" stance holds for v1/v2 but not KAS — KAS exposes `_kiro/hooks/{list,executeHook,triggerHook,…}`, so cyril can observe/list/trigger hooks (and the unified command+agent action model spans IDE+CLI authoring). A prime composable-stage opportunity. **Gating + direction resolved (2026-06-16):** the surface is **enabled by `clientCapabilities._meta.kiro.hooks = {enabled:true, v2:true}`** at `initialize` (a sibling of `_meta.kiro.settings` — *not* under it, and *not* a `~/.kiro/settings/cli.json` flag; both were verified negative). hooks are **server-run** (KAS executes command/agent actions in-process — no host-run callback; only `_kiro/hooks/cancel` pushes back, on `session/cancel`); the host role is to *enable + drive + observe* via `_kiro/hooks/{list,executeHook,triggerHook}` client→server requests. See the hooks section (incl. the unresolved hook-discovery caveat).
...
**Resolved 2026-06-16** (see "KAS live wire captures" above): KAS tool advertisement (`_kiro/tools/didChange` = category tags + per-MCP-tool); `session_info_update` `kind`-discriminator shape incl. turn-end/metering/context-breakdown; `_kiro/account/getUsage` message shape; hooks gating (enabled via client `_meta.kiro.hooks={enabled,v2}` at initialize — sibling of `settings`, *not* a cli.json flag) + direction (server-run, client-managed; hook *discovery* has an unresolved nuance); **the full `_kiro/*` notification catalog** — `governance/state`, `mcp/status`, `powers/items_changed`, `progressive_context/items_changed`, `steering/documents_changed` all fire on a plain default-settings turn (shapes captured).
