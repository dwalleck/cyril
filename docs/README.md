...
|---|---|---|---|
| [`kiro-2.24.0-wire-audit.md`](kiro-2.24.0-wire-audit.md) | 🧪 Research artifact | 2.24.0 (covers unaudited 2.23.0/2.23.1; 2026-09-24) | **v2 regression since 2.23.0**: `_kiro.dev/metadata` moved `effort` into `reasoning{…}` — cyril effort badge dead (cyril-q1xs). KAS 0.66.0→0.66.8 SAFE: `thinking` configOption, `session/delete`, `rejectionReason`, `_kiro/terminal/settings_changed` live-proven. Binaries stripped + zstd payloads; launcher resolves `kiro-cli-chat` via `PATH` (voids the 2.22.0 v2 pairing). |
| [`kiro-2.22.0-wire-audit.md`](kiro-2.22.0-wire-audit.md) | 🧪 Research artifact | 2.22.0 (covers unaudited 2.21.4; 2026-09-17) | Release wire audit. KAS 0.58.7→0.63.3→0.66.0 (ACP SDK ^1.3 bump, wire-identical); `focus_update.activity` (cyril-otpi); large tool results always offloaded; SDK bump inverted `set_model`/`session/delete` error codes. Its "v2 identical" claim is void — see the 2.24.0 audit § 2. |
| [`kiro-2.22.0-deserialization-audit.md`](kiro-2.22.0-deserialization-audit.md) | 🧪 Research artifact (focused) | 2.22.0 findings summary; examples 2.18.0, 2.19.1, 2.21.0 | Field-level comparison of current Cyril ACP types/converters against KAS MCP status, tool tags, permission metadata, `session_info_update` kinds, and other observed extension messages, with redacted examples. |
