#!/usr/bin/env python3
"""KAS 0.58.7 vs 0.66.0: does the ACP SDK ^0.19 -> ^1.3.0 bump (KAS 0.63.3,
kiro-cli 2.21.4) change how KAS answers standard session methods?

Static lead: the SDK's agent-method table lost `session/set_model` (present in
0.58.7, absent from 0.63.3+) and gained `mcp/connect|message|disconnect`,
`$/cancel_request`, `session/delete`. This probe sends the client->agent ones
and records the raw response (result or error code/message) per build.

    KAS=<acp-server.js> LABEL=<tag> probe-kas-sdk-methods-2.22.0.py <out.jsonl>

COST: zero real turns (no session/prompt).
"""
...
model_id = None
try:
    co = ((r or {}).get("result") or {}).get("configOptions") or []
    opts = next((c.get("options") or [] for c in co if c.get("id") == "model"), [])
    model_id = next((o.get("value") for o in opts if o.get("value") and o.get("value") != "auto"), None)
except Exception:
    pass
model_id = model_id or "claude-sonnet-4.5"
CALLS = [
    ("session/set_model", {"sessionId": sid, "modelId": model_id}),
    ("session/set_config_option", {"sessionId": sid, "configId": "model", "value": model_id}),
    ("session/set_model", {"sessionId": sid, "modelId": "bogus-model-xyz"}),
    ("session/list", {}),
    ("session/delete", {"sessionId": "sess_00000000-0000-0000-0000-000000000000"}),
    ("$/cancel_request", {"requestId": 999999}),
]
RESULTS = []
for method, params in CALLS:
    rid = req(method, params)
    resp = pump(rid, 30, "probe:" + method)
    RESULTS.append({"method": method, "params": params,
                    "response": None if resp is None else
                                ({"result_keys": sorted(resp["result"]) if isinstance(resp.get("result"), dict) else resp.get("result")}
                                 if "result" in resp else {"error": resp.get("error")})})
pump(-1, 3, "settle")
summary = {"probe": "result", "label": LABEL, "kas_version": kas_ver, "kas_pin": KAS,
           "model_id_used": model_id, "calls": RESULTS, "methods": dict(sorted(METHODS.items()))}
OUT.write(json.dumps(summary) + "\n"); OUT.flush()
print(json.dumps({"label": LABEL, "kas": kas_ver, "calls": RESULTS}, indent=1)[:4000])
try:
