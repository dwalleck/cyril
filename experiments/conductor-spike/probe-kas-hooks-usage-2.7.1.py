...
log("  (_meta.kiro.settings.hooks). Working enable path: _meta.kiro.hooks={enabled} (sibling of")
log("  settings) — see probe-kas-hooks-enabled-2.7.1.py. DIRECTION is a HOST-CALLBACK model per")
log("  @kiro/acp-type-covenant: the agent calls _kiro/hooks/{list,executeHook} back on the CLIENT,")
log("  which runs runCommand hooks. (processRunner in @kiro/agent is the no-client fallback.)")
