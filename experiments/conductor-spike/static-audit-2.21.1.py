#!/usr/bin/env python3
"""Static, read-only Kiro 2.21.1 vs 2.21.0 release audit.
...
NEW = Path(sys.argv[2]) if len(sys.argv) > 2 else Path("/home/dwalleck/.local/share/kiro-research/binaries/2.21.1/kiro-cli-chat")
OUT = Path(sys.argv[3]) if len(sys.argv) > 3 else Path("experiments/conductor-spike/static-2.21.1")
OUT.mkdir(parents=True, exist_ok=True)
CARVE = Path("/home/dwalleck/.local/share/kiro-research/kas-carves/2.21.1")
...
reports = {"old": collect("2.21.0", OLD), "new": collect("2.21.1", NEW)}
...
(OUT / "static-2.21.1-report.json").write_text(json.dumps(reports, indent=2, sort_keys=True), encoding="utf-8")
