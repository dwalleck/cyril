...
Variant: session/new carries _meta.kiro.settings.largeToolOutputHandler.enabled=true (the model-side offload handler is gated by isFeatureEnabled("largeToolOutputHandler"), default off).
...
TRACE = os.path.join(SCRATCH, "kas-output-transform-lth-2.19.2-trace.jsonl")
...
proc = subprocess.Popen([KIRO, "acp", "--agent-engine", "kas"], cwd=CWD, env=env,
                        ...
                        stderr=open(os.path.join(SCRATCH, "ot-lth-stderr.log"), "w"),
