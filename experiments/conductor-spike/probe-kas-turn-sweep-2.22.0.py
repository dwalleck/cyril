#!/usr/bin/env python3
"""v3/KAS paired wire sweep for the 2.22.0 audit (two scenarios, one skeleton).

KAS moved 0.58.7 (2.21.2) -> 0.63.3 (2.21.4, never audited) -> 0.66.0 (2.22.0),
so unlike 2.21.2 the KAS axis is live. Every leg runs through the INSTALLED
2.22.0 host and pins the bundle with KIRO_KAS_SERVER_PATH, isolating the
KAS-binary axis against one same-day backend.

    KAS=<acp-server.js> LABEL=<tag> [SCENARIO=turn|large] [TERM_CAP=on|off] \
        probe-kas-turn-sweep-2.22.0.py <out.jsonl>
