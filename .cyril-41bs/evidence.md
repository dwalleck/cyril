...
| P5 | Direct SDK 2.0.0 stable-wire-v1 must preserve real Kiro v2 and KAS behavior, not only compile against their callback catalog. | Do authenticated v2 and KAS sessions produce parity through the direct component? | PASS — authenticated lifecycle; KAS host families/permission/turn-end; v2 host callbacks named N/A; exhaustive callbacks separate |
| P6 | Official conductor topology must preserve real Kiro/KAS behavior through zero-proxy, no-op-proxy, and transforming-proxy chains in addition to passing deterministic full-duplex fixtures. | Do authenticated v2 and KAS sessions preserve initialization, extensions, callbacks, cancellation, failure, and ordering through all three chains? | PASS — live stable lifecycle across six cells; IDs/errors/cancellation proven by deterministic conductor fixtures, not live defaults |
...
- Live scope: `e5` and `e6_live` create temporary directories; v2 sends an inert text prompt, while KAS reads the temporary README and runs `printf SDK2_HOST_CALLBACK` to elicit host callbacks. They perform no repository mutation, credential logging, persistent state writes, or consequential action.
...
- Live oracle: after the user refreshed the Kiro token, `oracles/e6_live_parity.py` ran direct v2/KAS and all six conductor cells, required stable protocol/end-turn/lifecycle milestones, required KAS auth/filesystem/terminal/permission/hooks plus structural turn-end ordering, enforced v2's named host-callback N/A, and returned `direct_vs_conductor_equivalent: true`. The deterministic direct/conductor matrix separately proved all 18 requests, two notifications, actual response IDs, typed-error data, and transform payloads. No credential value was printed.
...
| P5 | The direct SDK client completed authenticated Kiro v2 and KAS sessions on protocol v1. KAS exercised auth, filesystem, terminal, permission, and hooks callbacks with permission before terminal creation and structural `turn_end` before the prompt response; v2's unelicited host callbacks are an explicit N/A. Live typed-error, outer-response-ID, and cancellation fields are explicitly `not_exercised`. The separate deterministic direct matrix answered all 18 callback requests and two hook notifications with observed wire IDs and exact typed-error data. | Covenant plus direction-checked committed captures supplied versioned extension references only; the current live sessions supplied lifecycle/KAS-family evidence, and the deterministic SDK matrix supplied exhaustive callback evidence. | PASS — split live/deterministic/capture evidence |
| P6 | All six v2/KAS × zero/no-op/transform live conductor cells negotiated protocol v1, returned a session ID, ended with `stop_reason: end_turn`, and matched their direct engine's exact stable lifecycle milestones. KAS cells exercised every required host family and turn/permission ordering; v2 cells retained the named N/A. Separate deterministic conductor matrices preserved actual IDs, typed errors, callback cardinality, transformed payloads, and declared markers; `e6` separately preserved cancellation, failure, direction, and distinct/repeated order. | Five official conductor initialization/cancellation tests passed; the independent Python comparator enforced exact cell sets, lifecycle milestones, evidence provenance, observed IDs, and mutation controls. | PASS — live lifecycle plus deterministic failure/identity/cancellation |
| P7 | In the refreshed real v2 and KAS transforming chains, the proxy annotated outbound `session/prompt` and inbound vendor notifications (`_kiro.dev/metadata` or `_kiro/mcp/status`); both agents still ended normally. The deterministic matrix independently observed `_meta.cyrilProbeObserved` on all 18 transformed callback requests. Engine conversion and host callback effects stayed in the terminal client/agent components. | Independent bidirectional envelope algebra transformed request, response, and notification values, restored outer response identity, and confirmed no Engine/host owner in either proxy binary or the shared helper. | PASS |
