...
The resolved shell is moved to the bridge thread as `Option<HostShell>` inside a private resolved-startup bundle: `None` is the real V2 absence and `Some` is produced only after KAS resolution. `KiroClient` stores that immutable option beside the existing terminal registry. Its shell-type and terminal-create callbacks both require and borrow the same `Some(HostShell)`; the bridge constructor is the only production caller and tests fence the engine/option pairing. This keeps one snapshot without adding host-execution concerns to the existing `Engine` conversion/capability seam or changing the many `KasEngine` conversion fixtures.

### Response and execution

`KiroClient` serializes `HostShell::wire_name()` for shell type and passes the same `&HostShell` into `TerminalRegistry::create`. The registry asks `HostShell::command()` for the process, then applies the existing cwd translation, request environment, null stdin, piped stdout/stderr, and `kill_on_drop` lifecycle. V2’s `None` arm returns the ACP default/refusal path and never reaches either KAS callback.
