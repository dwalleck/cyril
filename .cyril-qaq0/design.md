...
|---|---|---|---|
| Bundled catalog, id spelling, mode spelling, detection precedence | `crates/cyril-ui/src/theme.rs` | None — extends the existing `resolve`/`ThemeId` seam with `parse_theme_id`, `parse_color_mode`, `detect_color_mode`, `ColorModeRequest`, `ColorEnvironment`, `StartupAppearance`, `AppearanceDiagnostic` | No env read, no config read, no rendering |
...
|---|---|---|---|---|---|
| `crates/cyril-ui/src/theme.rs` | `+ parse_theme_id`, `parse_color_mode`, `detect_color_mode`, `ColorEnvironment`, `ColorModeRequest`, `StartupAppearance`, `AppearanceDiagnostic` | catalog spelling, mode spelling, detection precedence | precedence table, accepted sets | env reads, config, rendering | `deepen` |
...
| `crates/cyril/src/app.rs` | `App::new(+ StartupInputs { ui, environment })` | UiConfig consumption, result routing, picker Enter branch | — | env reads, palette data | `widen` |

Protected parents:

| Protected parent | Baseline | Allowed change | Forbidden | Exit condition |
|---|---|---|---|---|
| `crates/cyril-ui/src/widgets/` | render from `&Theme` only | one test-fixture line in `widgets/picker.rs` (widening `PickerState` with `kind` forces it) | any `ThemeId`/`ColorMode`/`resolve` reference in production | existing widget source fence passes |
...
- **C1** Every accepted `[ui] theme` string maps to its `ThemeId`; every other
  string maps to `None`.
- **C2** Every accepted `[ui] color_mode` string maps to its request — a fixed
  `ColorMode` or `Automatic` — and every other string maps to `None`. *(Refined
  2026-09-09 during slice 2: `automatic` is not itself a `ColorMode`, so the
  parse result is a `ColorModeRequest`.)*
...
|---|---|---|---|---|---|---|---|---|
| C1 | Accepted theme ids map; others do not | `[ui] theme` × 10 shapes | `theme::tests::parse_theme_id_covers_exactly_the_bundled_ids` asserts all 6 accepted ids round-trip and 5 rejected strings return `None`; a wrong mapping falsifies. Control: a lenient parser passes the accepted half but fails the rejected half | `.cyril-qaq0/oracle-theme-ids.py` computes the accepted set from the spec table, independent of the Rust macro | make `parse_theme_id` return `Some(ThemeId::CyrilDark)` for unknown input | `theme::tests::parse_theme_id_covers_exactly_the_bundled_ids` | seconds | PASS — 2026-09-09, mutation M2 red (`"" must be rejected`), restored green; `compare_appearance.py` PASS 30 rows |
| C2 | Accepted mode strings map; others do not | `[ui] color_mode` × 10 shapes | `theme::tests::parse_color_mode_covers_exactly_the_five_values`; a wrong mapping falsifies | `.cyril-qaq0/oracle-theme-ids.py` (same independent table) | make `parse_color_mode` accept any unknown string as `automatic` | `theme::tests::parse_color_mode_covers_exactly_the_five_values` | seconds | PASS — 2026-09-09; `compare_appearance.py` PASS 30 rows covers the accepted and rejected sets |
| C3 | Detection follows the precedence table | env × 4 vars × explicit/absent | `theme::tests::detection_precedence_matches_every_table_row` drives 14 injected rows; any row mismatching the spec table falsifies. Control: a detector that ignores `NO_COLOR` fails the no-color rows; one that ignores the explicit value fails row 1 | `.cyril-qaq0/oracle-precedence.py` implements the table independently and prints the expected mode per row | move the `NO_COLOR` check above the explicit-config check | `theme::tests::detection_precedence_matches_every_table_row` | seconds | PASS — 2026-09-09, mutation M3 red (`explicit-beats-no-color resolved to the wrong mode`), restored green; oracle PASS 14 cases |
| C4 | Preview drives the rendered theme | picker open/closed × selection | `state::tests::theme_preview_drives_the_rendered_theme_until_commit` (filtered selection previews, committed value unchanged, commit lands) plus `state::tests::picker_cancel_discards_the_preview`; a preview that does not reach `TuiState::theme` falsifies | The renderer reads only `TuiState::theme`; the surface check rendered the real picker and observed the border color move `Rgb(86,199,208)` → `Rgb(142,192,124)` | make `TuiState::theme` ignore the preview | `state::tests::theme_preview_drives_the_rendered_theme_until_commit` | seconds | PASS — 2026-09-09, mutation M10 red (`the filtered selection must preview`), restored green; real-surface render verified |
| C5 | Enter commits, Esc discards, neither writes | picker lifecycle | `app::tests::theme_picker_enter_commits_and_esc_discards_without_bridge_traffic` runs open→move→Esc and open→move→Enter, asserting the committed palette and zero bridge traffic; `app::tests::theme_picker_session_leaves_config_bytes_untouched` loads `.cyril-qaq0/config-nowrite-fixture.toml` (comments, alignment, inline comment), runs a full session, and requires byte-identical content. Positive control: `state::tests::commit_theme_preserves_the_active_color_mode` and `commit_theme_is_inert_for_an_agent_picker` | The fixture's bytes, read before and after, independent of any serializer | keep the preview on `picker_cancel` | `state::tests::picker_cancel_discards_the_preview` | seconds | PASS — 2026-09-09, mutation M9 red (`Esc must restore the committed theme`), restored green |
| C6 | Every theme × mode renders resolved roles | 6 ids × 4 modes × 4 scenes | Extended `all_scene_theme_mode_combinations_pass` (96 passes) — **cheapest falsifier, run 2026-09-09: PASS**; a theme whose roles are not the ones rendered falsifies | The pinned 7,680-cell Cyril Dark baseline plus per-role projection computed from `resolve(id, mode)`, independent of the renderer | in `theme::resolve_no_color`, leak the `text` role's truecolor value instead of `Reset` (`mutations.sh` M1). *(Corrected 2026-09-09: the original mutation — `resolve` ignoring its `id` — was verified **unobservable**, because the fence renders the marker and the projected scene through the same code path, so a symmetric change cancels.)* | `render::tests::all_scene_theme_mode_combinations_pass` | seconds | PASS |
| C7 | No-config startup is byte-identical | absent config | `migrated_scenes_match_all_pinned_cells` (7,680 cells, 0 differences) plus `state::tests::new_state_uses_cyril_dark_truecolor` and `app::tests::absent_appearance_keys_are_silent`; a changed default falsifies | The committed fixture `crates/cyril-ui/tests/fixtures/conversation-theme-baseline.tsv`, captured before this change | change `UiState::new`'s default to another palette. *(Corrected 2026-09-09: the baseline fence resolves its theme directly via `truecolor_theme()`, so this mutation cannot move it — the observable fence is `new_state_uses_cyril_dark_truecolor`.)* | `state::tests::new_state_uses_cyril_dark_truecolor` | seconds | PASS — 2026-09-09, mutation M7 red, restored green; baseline 0/7,680 differences |
| C8 | Unknown value → one visible message | unknown theme; unknown mode | `app::tests::unknown_theme_value_reports_one_visible_message` and the mode twin assert exactly one message naming the key, value, and default; zero or two messages falsifies. Positive control: `absent_appearance_keys_are_silent` asserts zero messages | The expected string is composed from the spec's message shape, not from the implementation's constant | drop the diagnostic push | `app::tests::unknown_theme_value_reports_one_visible_message` | seconds | PASS — 2026-09-09, mutation M4 red, restored green |
| C9 | `/theme` sends no bridge command | command dispatch | `commands::theme_command_tests::theme_command_is_local_and_returns_show_theme_picker` and `app::tests::theme_command_opens_picker_without_bridge_traffic` (the test bridge records zero sends); a query falsifies. Positive control: `/new` on the same receiver records one command | The test bridge's send log, independent of the command implementation | make the theme command send a bridge command | `app::tests::theme_command_opens_picker_without_bridge_traffic` | seconds | PASS — 2026-09-09, mutation M8 red (`/theme must not reach the agent`), restored green; `.cyril-qaq0/oracles/module_shape.py` also asserts the `/theme` regions construct no `BridgeCommand` |
| C10 | Catalog knowledge stays in `cyril-ui` | placement | `.cyril-qaq0/oracles/module_shape.py` asserts no `ThemeId`/`ColorMode`/bundled id literal in `cyril-core`/`cyril-memory` production and no palette access in widgets; a violation falsifies. Positive control: `--selftest` reports a planted `ThemeId` reference and accepts a clean file | Source census independent of the compiler | leak a bundled id literal into `cyril-core` production | `.cyril-qaq0/oracles/module_shape.py` | seconds | PASS — 2026-09-09, mutation M6 red (`bundled id literal "gruvbox-dark"`), restored green; `--selftest` PASS |
| C11 | Wrong type keeps whole-file defaults | `theme = 3`; `color_mode = true` | `config::tests::wrong_typed_theme_falls_back_to_whole_file_defaults` and the mode twin, with a non-default sibling key (`max_messages = 1000`) as the discriminator; per-key survival falsifies | The existing nd4h legacy-compat test shape, independent of the new fields | add `#[serde(deserialize_with)]` that silently defaults the field | `config::tests::wrong_typed_theme_falls_back_to_whole_file_defaults` | seconds | PASS — 2026-09-09, mutation M5 red (`rejection must be whole-file, not field-skipping`), restored green |
...
- 2026-09-09 — **C6 mutation corrected during the slice-1 gate.** The design's
  original mutation (`theme::resolve` ignoring its `id`) was applied and the
  extended fence stayed **green**: the fence renders a marker scene and a
  projected scene through the same code path and compares them, so a change
  that alters both identically is unobservable. The named mutation is now a
  role leak — `resolve_no_color` returning the `text` role's truecolor value
  instead of `Reset` — which the fence reports as
  `CyrilDark/no-color/markdown foreground cell 122: syntax expected Reset, actual Rgb(255, 255, 255)`.
  Restored → green. This is a technical proof correction, not a design change.
