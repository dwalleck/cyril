...
use crate::types::{
    ...
    TurnMeteringUpdate, UsageAccount, UsageAccountBreakdown, UsageBonusCredit, UsageTurnStatus,
};
...
pub(crate) fn account_usage_from_response(
    ...
    let data = response
        .get("data")
        .ok_or(AccountUsageParseError::Missing("data"))?;
...
fn required_percentage(
    ...
    let percentage = required_nonnegative(value, field)?;
    if percentage > 100.0 {
        return Err(AccountUsageParseError::Invalid(field));
    }
    Ok(percentage)
