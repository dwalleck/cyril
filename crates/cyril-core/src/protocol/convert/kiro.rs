...
/// kiro-cli's in-label encoding of "this is the active option".
const ACTIVE_LABEL_SUFFIX: &str = "[active]";

/// Split kiro-cli's `"High  [active]"` label into its display text and the
/// current-option bit it smuggles (cyril-imjx).
///
/// `/effort` options carry no `current` field; after an in-session change the
/// active level is marked by appending [`ACTIVE_LABEL_SUFFIX`] to the label.
/// That is a wire encoding, not label text, so it is decoded here — the
/// renderer draws its own marker and must never see the suffix. Only a
/// *trailing* occurrence is a marker; a label that merely mentions the token
/// keeps it.
fn split_active_label_suffix(label: &str) -> (&str, bool) {
    match label.trim_end().strip_suffix(ACTIVE_LABEL_SUFFIX) {
        Some(stem) => (stem.trim_end(), true),
        None => (label, false),
    }
}
...
/// additionally joins against cyril's own session state — see
/// `UiState::show_picker`.
pub(crate) fn parse_options_response(response: &serde_json::Value) -> Vec<CommandOption> {
    ...
    opts.iter()
        .filter_map(|opt| {
            ...
            let (label, label_marks_active) = match opt.get("label").and_then(|v| v.as_str()) {
                Some(label) => {
                    let (label, marks_active) = split_active_label_suffix(label);
                    (label.to_string(), marks_active)
                }
                None => (
                    opt.get("name")
                        .and_then(|v| v.as_str())
...
pub(crate) fn to_ext_notification(
    ...
    match method {
        "kiro.dev/metadata" => {
            ...
            // object still alerts (fallback text downstream). The stopReason
            // literal is exact (case-sensitive); wire keys are provisional
            // until the first live refusal capture (cyril-pz51). JSON `null`
            // for the object or a subfield means "not provided" (Kiro's own
            // `r?.field` / `?? fallback` chain treats null and undefined
            // identically), so null is absent, not corrupt.
            let content_filtered = match params.get("stopReason") {
                None => false,
                Some(v) if v.is_null() => false,
                Some(v) => match v.as_str() {
                    Some(s) => s == "CONTENT_FILTERED",
                    None => {
                        tracing::warn!(
                            value = ?v,
...
mod tests {
    ...
    /// cyril-imjx: kiro-cli encodes the active `/effort` level into the option
    /// LABEL (`"High  [active]"`) instead of sending `current: true`. That is a
    /// wire quirk, so it is normalized here: the suffix becomes `is_current`
    /// and never reaches the renderer as label text. Shape captured live at
    /// 2.4.1 (`experiments/conductor-spike/trace-2.4.1-tui-recorder.jsonl`).
    #[test]
    fn parse_options_response_folds_active_label_suffix_into_is_current() {
        let response = serde_json::json!({
            "options": [
                {"value": "low", "label": "Low"},
        ...
        assert_eq!(
            opts[2].label, "High",
            "the [active] suffix is a marker, not label text"
        );
        assert!(opts[2].is_current, "the suffix means this row is current");
        assert_eq!(
            opts.iter().filter(|o| o.is_current).count(),
            1,
            "exactly one row is current"
        );
    }

    /// The suffix is only a marker when it is a suffix — a label that merely
    /// mentions it keeps its text.
    #[test]
    fn parse_options_response_keeps_non_suffix_active_text() {
        let response = serde_json::json!({
            "options": [{"value": "none", "label": "[active] only when trailing"}]
        });
        let opts = parse_options_response(&response);
        assert_eq!(opts[0].label, "[active] only when trailing");
        assert!(!opts[0].is_current);
    }
    ...
    #[test]
    fn parse_options_response_preserves_active_suffix_in_value_fallback() {
        let response = serde_json::json!({
            "options": [{"value": "mode[active]"}]
        });
        let opts = parse_options_response(&response);
        assert_eq!(opts[0].label, "mode[active]");
        assert!(!opts[0].is_current);
    }
    ...
    #[test]
    fn parse_options_response_preserves_active_suffix_in_name_fallback() {
        let response = serde_json::json!({
            "options": [{"value": "mode", "name": "Mode [active]"}]
        });
        let opts = parse_options_response(&response);
        assert_eq!(opts[0].label, "Mode [active]");
        assert!(!opts[0].is_current);
