...
| 7 | timeout parse | file `timeout: 2` on `sleep 1 && echo ok` → included (catches secs-as-millis); absent → HookDef reports 60s | file schema (kasHookFileSchema: seconds) | 10m | pending | `session_start_timeout_seconds_not_millis` + `hook_def_default_timeout` |
