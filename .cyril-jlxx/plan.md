...
Partition: archive PR #117 contains 3,454 lines plus a 300-line margin = 3,754. It independently retains historical proof, instruments and roadmap ownership; 20 JSON documents, six Python syntax checks and the baseline/mutated shape oracle validate it without the feature. The separate `core-review-runtime` increment descends from #117: its first assembled delta is 1,446 lines, projected at 1,700 after the grouped-choice loss fence, lifetime fixture correction and checkpoint record. It contains no workbench crate/dependency and must pass its own core/library, process and configuration proofs. PR #116 is based on that core increment; project 3,300 workbench/current-proof lines plus 300 churn = 3,600. Recompute each actual delta before publishing; no increment may exceed 4,000.
