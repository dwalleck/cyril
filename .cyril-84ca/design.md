...
5. **C5 — ordering:** every streaming notification of a turn is emitted before that turn's `TurnCompleted`. **RESOLVED (Slice 7): verified empirically, not a cyril-84ca property.** Holds for real v2 AND KAS (`.cyril-84ca/c5-ordering-evidence.log`); the off-loop change neither causes nor regresses it (the pre-fix inline `conn.prompt().await` shared the identical library behavior). A deterministic unit fence is unattainable: the only reorder is a theoretical zero-gap `agent-client-protocol` race (responses resolve inline, notifications queue via `incoming_tx`), filed as **cyril-9akh**. Evidence = the `notif_order` probe; no CI unit fence.
...
| C5 | ordering | Drive cyril's real bridge against a backend; stream a turn; observe notification order. If `TurnCompleted` precedes a streamed `AgentMessage`, false. **Buggy impl:** task emits `TurnCompleted` before the io_task drains updates. | `notif_order` probe vs real kiro (independent of the unit harness) | 2m (empirical) | **resolved — holds for v2 + KAS** | empirical probe `.cyril-84ca/notif_order.rs`; zero-gap library race tracked at **cyril-9akh** (not a CI unit fence — see C5 claim) |
